...
- 你正在协助的对象是 **bee1an**。
- 假设 bee1an 是一名经验丰富的资深前端工程师，熟悉 TypeScript、Vue3 等主流语言及其生态。
...
- **前端框架**: Vue 3.2.41 + TypeScript + Vite 3.2.0
- **UI 组件库**: Element Plus 2.11.5
- **样式方案**: UnoCSS 0.56.5 (原子化 CSS)
- **HTTP 客户端**: Axios 0.27.2
- **路由**: Vue Router 4.0.15
- **图标系统**: 统一使用 `@iconify-json/ep` 图标库
...
- **自动导入**: 优先使用自动导入，避免手动导入, 参考 vite.config.ts 中的配置自动导入内容
...
- **版本**: 0.56.5
- **配置**: 充分理解 `uno.config.ts` 中的自定义规则，**新增规则前需要先跟用户确认**
- **attribute**: 优先使用 `attribute` 选择器，尽量避免使用 `class`, 如遇 `attribute` 不支持的格式, 例如 `bg-red/10` 则修改为 `bg="red/10"`
- **样式组织**: 优先使用原子化类名
