...
### Requirement: Iframe Detection Process
The iframe detection process SHALL maintain its current functionality while using the new authentication mechanism.

#### Scenario: Single iframe detection
- **WHEN** the system detects exactly one iframe on the page
- **THEN** the system SHALL automatically attempt to obtain an openKey
- **AND** process the iframe URL with the new authentication
- **AND** provide the same user feedback as before

#### Scenario: Multiple iframe selection
- **WHEN** the system detects multiple iframes
- **THEN** the system SHALL display the iframe list for user selection
- **AND** upon selection, attempt openKey retrieval for the chosen iframe
- **AND** process the URL with the obtained openKey
- **AND** each iframe requires a separate openKey API call (no caching)

#### Scenario: Batch processing multiple iframes
- **WHEN** the system needs to process multiple iframes
- **THEN** the system SHALL make individual openKey API requests for each iframe
- **AND** not attempt to cache or reuse openKeys between different iframes
- **AND** handle each iframe's authentication independently

## REMOVED Requirements

### Requirement: SessionStorage Access
**Reason**: This approach is being replaced by API-based authentication to improve reliability and security.
**Migration**: All sessionStorage access logic will be removed and replaced with openKey API calls.
